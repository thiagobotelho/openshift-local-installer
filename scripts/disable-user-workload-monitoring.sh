...
if grep -qE '^enableUserWorkload:' "${config_file}"; then
  sed -i -E 's/^enableUserWorkload:.*/enableUserWorkload: false/' "${config_file}"
else
  if [[ -s "${config_file}" ]]; then
    printf '\n' >>"${config_file}"
  fi
  printf 'enableUserWorkload: false\n' >>"${config_file}"
fi
...
echo "[OK] User Workload Monitoring desativado em openshift-monitoring/cluster-monitoring-config."
echo "[INFO] O monitoramento de workloads desta stack fica sob responsabilidade do prometheus-gitops/MonitoringStack."
